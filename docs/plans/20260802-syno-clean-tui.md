...
- [x] **if the Task 5 fixture is still marked PROVISIONAL, re-capture it from the real NAS now** with `--dump-tasks-json` and re-run the parser tests against the real response; also confirm the discovered API versions with `--dump-api-info` — ⚠️ **NOT DONE AND NOT DOABLE HERE. The fixture remains PROVISIONAL.** No Synology NAS is reachable from the execution environment, so neither dump flag can be run. The `_comment` marker and `model.rs`'s `the_fixture_is_still_marked_provisional_until_task_21_recaptures_it` are **deliberately left in place** so the debt cannot be lost. **Re-capture against a real DSM 7 NAS before any release**, re-run the parser tests, and confirm the discovered API version ranges (`SYNO.API.Auth`, `SYNO.DownloadStation.Task`, `SYNO.FileStation.List`, `SYNO.FileStation.Delete`) against `AUTH_SUPPORTED`/`DS_TASK_SUPPORTED`/the File Station ranges. Everything below was verified against the provisional fixture, so any check that depends on the *wire shape* is only as good as that file.
- [x] verify every requirement from Overview: table with full stats, arrow navigation, spacebar multi-select, `d` → confirmation listing selections → deletes both task and files, sort options, status filters, live refresh, pause/resume — all traced to code: 11 columns in `table::COLUMNS`; `↑↓kj/PgUp/PgDn/Home/End/g/G` in `App::handle_normal_key`; `Space`→`toggle_selection`, `a`, `Esc`; `d`→`begin_delete`→`Mode::Confirm`→`ui::dialog::render_confirm`→`take_confirmed_delete`→`event::spawn_delete` (pause → File Station recursive delete → DS task delete); `s`/`S`/`f`/`/`; `event::spawn_poller`; `p`/`u`→`event::spawn_task_op`. ⚠️ **One narrowing:** the Overview says "sort by any column" but `SortKey::ALL` has 8 keys against 11 columns — Seeds/Peers, ETA and Destination are not sortable, and `Added` sorts by a value that has no column. This follows Task 7's enumerated `SortKey`, which is the more specific spec and was implemented as written; it is a documented gap, not a regression. Post-v1 if wanted.
- [x] verify edge cases: zero tasks, a single task, 500+ tasks via `--fixture` (scrolling and render performance), very long CJK/emoji titles (column alignment holds), a task with zero size, a paused task with no speed, network drop mid-session (banner appears, recovers on reconnect), a task whose files were already removed manually (skipped cleanly, task still deleted) — driven through a throwaway `TestBackend` harness over generated 0/1/600-task fixtures in a temp dir (`tests/fixtures/task_list.json` was **not** touched). Results: zero tasks renders "No Download Station tasks" and every frame fits at 1x1/40x10/80x24/200x60; a single task never lets the cursor leave row 0 under any movement key; 600 tasks parse through the real `--fixture` loader and render in **3.7 ms/frame** release (36 ms debug) at 160x50, with `End` landing on the last row and the window pinned to the last full page; CJK and emoji titles hold alignment — ➕ **new permanent test** `ui::table::tests::every_column_lines_up_at_every_terminal_width` asserts every composed span is exactly its column width at 60/80/100/120/160/200 cells (the old test only covered 120, where nothing truncates), and ➕ `ui::tests::a_list_far_longer_than_the_screen_scrolls_and_still_fits` covers the 500+ case permanently. The zero-size task yields `progress() == 0.0` with no NaN/inf in any cell; the paused task renders `—`/`—` for both speeds and `∞` for ETA (zero and unknown stay different sentinels). Poll failure is non-fatal by construction — `event::poll_once` returns a `bool` and the poller only stops on a **closed channel**, never on an error — and recovery is covered by `app::tests::{a_failed_poll_raises_a_banner_without_disturbing_anything_else, the_next_successful_refresh_takes_the_banner_down}`. Files already removed by hand: `event::run_op` maps `PathInfo::Missing` to `OpOutcome::NothingThere`, which does **not** stop the loop, so `Op::DeleteTask` still runs and the item reports "the files were already gone; the task was removed". ⚠️ The `Missing` branch itself needs a live File Station reply and is verified **by inspection plus `api::file_station`'s `classify_getinfo` tests**, not end to end.
- [x] verify the safety path end to end: `--dry-run` issues no destructive calls; a file list with no common root is skipped rather than guessed at; `--no-delete-files` leaves files intact; deleting an actively seeding task pauses it first — `--dry-run`: all five destructive call sites live in `event.rs` (`delete_paths`, `delete_tasks`, `pause_tasks` ×2, `resume_tasks`) and every one is behind a `dry_run` early return; ➕ **new test** `event::tests::a_dry_run_delete_issues_no_call_at_all_and_claims_no_successes` runs the whole three-phase executor over the entire fixture against a client whose host does not resolve, so `failed: 0` is positive proof **nothing was sent** — and it reports every item as *skipped*, never as a success. No common root: `delete::resolve_delete_path` refuses `dbid_013`, `plan_delete_ops` returns an **empty** op list for it under all three option combinations, and ➕ **new test** `event::tests::a_refused_item_reaches_the_nas_not_even_for_its_task_delete` proves the executor issues nothing at all for it in a **real** (non-dry) run — the DSM task is not deleted either, so the data is never orphaned. `--no-delete-files` yields exactly `[Op::DeleteTask]` for every deletable fixture item — no pause, no file delete. Active tasks: `plan_delete_ops` puts `Op::Pause` first for every status except Paused/Finished/Error, `Op::DeleteTask` is always last, and `event::pause_and_confirm` re-reads `getinfo` until the task actually reports itself paused (bounded at 15 s). ⚠️ The `--dry-run` **CLI flag → `DeleteOptions`** wiring and the live pause/delete round trips are traced by inspection (`config::merge` → `ResolvedConfig::dry_run` → `DeleteOptions::from_config`); only the executor's behaviour given those options is under test.
- [x] run the full suite: `cargo test --all` — **483 passed, 0 failed** (479 before this task; +4 new).
- [x] run `cargo fmt --all -- --check` and `cargo clippy --all-targets -- -D warnings` — both clean, as is `cargo build`.
- [x] confirm every module listed under Testing Strategy as "tested" actually has tests — all seven present, counted from the test names: `app` 113, `delete` 69, `view` 30, `api::client` 26, `model` 25, `format` 21, `error` 18. (Also tested beyond the strategy's list: `config` 42, `ui` 34, `ui::table` 25, `api::file_station` 22, `ui::dialog` 18, `event` 15, `api::download_station` 11, `api::auth` 8, `cli` 6.)

⚠️ **Verification notes for Task 21** (what was actually done, and what could not be):
- 🔺 **No NAS, no TTY.** Nothing in this task ran against Download Station or an interactive terminal. Rendering was verified with ratatui's `TestBackend` (the pattern the whole suite already uses) and non-interactive behaviour by running the binary: `--version`, `--help`, `--fixture` on a 600-task file (parses, then exits **1** on the TTY check with nothing on stdout), a missing fixture (exit 1, names the file), a clean `$HOME` (exit 1, writes the starter config template and lists both missing values), and `--host nas.invalid.example --dry-run` (exit 1, three-line connection diagnostic naming the URL and the hint).
- ➕ **Four tests added, no product code changed.** The verification found **no defects in the implementation**. Two of the four close real coverage gaps the earlier tasks left: the delete executor's dry-run and refused-item paths had never been exercised as a whole (only their pure inputs had), and column alignment had only ever been asserted at one terminal width, where no title is truncated. The other two are the 500+ row case and its scroll window.
- ➕ `ui::tests::frame_text_narrow`. The existing `frame_lines` helper concatenates every buffer cell, including the space ratatui parks in the *continuation* cell of a double-width glyph and never emits — so a CJK title read back out of it has a space after every character and `contains` cannot match. That is a property of the test helper, not of the renderer (the same space is what makes the existing "one character per cell" width check correct), but it is a trap: the new helper drops continuation cells and says why. The first draft of the 500-row test failed on exactly this and looked like a real bug.
- The throwaway harness (`tests/acceptance.rs`) and its generated fixtures lived in a temp dir and were **deleted**; only the four in-module tests remain. `tests/fixtures/task_list.json` is byte-identical to what Task 5 wrote.
- **`--logout` is not suppressed by `--dry-run`.** Noticed and deliberately left: ending a session destroys no data, and the flag is explicit. Worth a line in the README only if it ever surprises someone.

### Task 22: [Final] Update documentation

- [x] update `README.md` with a real screenshot/asciinema recording of the working TUI — ⚠️ **a real terminal screenshot/asciinema capture is still outstanding and must be added before publishing.** There is no TTY in this environment, so nothing could be photographed and nothing was fabricated (no invented asciinema URL, no fake image). What landed instead is honest and useful: two **rendered frames** produced by the program's own `render()` through ratatui's `TestBackend` against `tests/fixtures/task_list.json` — a 120x17 table frame with two rows selected at the top of the README, and the delete confirmation modal in the "What you see is what goes" section — each embedded as a fenced text block and labelled explicitly as a rendering of fixture data rather than a photograph of a live session. The frames were captured with a throwaway `tests/render_frame_scratch.rs` harness that was **deleted** afterwards; no permanent test or product code changed.
- [x] finish `CLAUDE.md` (stubbed in Task 1): module layout, CLI/env/file precedence, the three-phase delete ordering, and the path-safety invariants — all four present and expanded. Rewritten end to end rather than appended to: the "sections marked _(pending)_" preamble is gone (nothing is pending), a table of contents was added, and the mis-nested `###` sections (formatting and the fixture were filed under "DSM API conventions") were lifted to their proper places. New structure: what this is → toolchain/validation gate → dependency rules → module layout → configuration precedence → error handling → DSM API conventions → **the dangerous part** (delete ordering + path safety, now one section instead of two) → UI and state conventions → formatting → offline/debugging flags → testing philosophy → **➕ known gaps and outstanding debt**. Every identifier it names was grepped against `src/` before the rewrite; the crossterm/`EventStream` note is now stated once (dependency rules) and cross-referenced rather than repeated in three places.
- [x] finalize the `CHANGELOG.md` 0.1.0 entry — the seeded entry was rewritten against the shipped feature set and grouped (the table / deleting / live operation / connecting and configuring / project), Keep a Changelog format retained. ➕ A **Known limitations** subsection was added — DSM 7 only, both DSM packages required, 8 sort keys against 11 columns, no responsive column dropping, the `#recycle` hazard, `--logout` not covered by `--dry-run` — so the GitHub release notes (which the release workflow extracts from this section verbatim) carry them too. Two inaccuracies were corrected: "sort by any column" became the real eight keys, and the per-task-failure detection inside `success: true` envelopes was added. `## [0.1.0] - unreleased` is kept deliberately with an HTML comment telling the tagger to fill the date in — inventing a release date for an untagged version would be a fabrication, and the comment does not render in the release notes.
- [x] move this plan to `docs/plans/completed/` — **(deferred to harness — moved after review phases)**

⚠️ **Decisions taken during Task 22** (plan text above kept verbatim; actuals recorded here):
- ➕ **Three README accuracy fixes beyond the checkbox list.** "Sort by any column" contradicted Task 21's finding (8 `SortKey`s against 11 `COLUMNS`) and now names the eight keys and the three columns that are not sortable. "Each archive is published with a matching `.sha256` checksum" contradicted `release.yml`, which concatenates the per-target files into a single `SHA256SUMS` and deletes them (`cat ./*.tar.gz.sha256 > SHA256SUMS && rm -f`). The screenshot placeholder's HTML comment promising "a real capture before v0.1.0" was replaced by the rendered frames plus an explicit outstanding-work warning, so the debt is visible to a reader rather than buried in a comment.
- **No product code was touched.** The verification found nothing to fix; documentation was the whole task. `cargo build`, `cargo test` (483 passed), `cargo fmt --all -- --check` and `cargo clippy --all-targets -- -D warnings` are all clean.
- The `⚠️` debt items are now recorded in **three** places on purpose, each for a different reader: `CLAUDE.md`'s "Known gaps" section (the next contributor), the CHANGELOG's "Known limitations" (the user reading release notes), and this plan (the record). The provisional fixture, the missing real screenshot, the placeholder `https://github.com/emacarov/syno-clean` URL and the never-run-against-a-NAS status all appear in the first of those.
